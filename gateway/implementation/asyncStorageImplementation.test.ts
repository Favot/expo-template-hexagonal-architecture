...
import type { StorageKey } from "../services/storageClient";
...
describe("secureStorageClient web", () => {
	...
	it("should throw error when localStorage throws on getItem", async () => {
	...
	it("should throw error when localStorage throws on setItem", async () => {
		const error = new Error("Storage error");
		jest.spyOn(localStorage, "setItem").mockImplementation(() => {
			throw error;
		});
		await expect(
			secureStorageClient.setItem(testKey, testValue),
	...
	it("should throw error when localStorage throws on removeItem", async () => {
		const error = new Error("Storage error");
		jest.spyOn(localStorage, "removeItem").mockImplementation(() => {
			throw error;
		});
		await expect(secureStorageClient.removeItem(testKey)).rejects.toThrow(
			error,
		);
	});
	...
	it("should throw error when localStorage throws on clear", async () => {
		const error = new Error("Storage error");
...
describe("secureStorageClient native", () => {
	...
	it("should return null when item does not exist in SecureStore", async () => {
		const mockGetItem = jest.mocked(SecureStore.getItemAsync);
	...
	it("should throw error when SecureStore throws on getItem", async () => {
		const mockGetItem = jest.mocked(SecureStore.getItemAsync);
		mockGetItem.mockRejectedValueOnce(new Error("SecureStore error"));

		await expect(secureStorageClient.getItem(testKey)).rejects.toThrow(
			"SecureStore error",
		);
		expect(mockGetItem).toHaveBeenCalledWith(testKey);
		expect(mockGetItem).toHaveBeenCalledTimes(1);
	...
	it("should throw error when SecureStore throws on setItem", async () => {
		const mockSetItem = jest.mocked(SecureStore.setItemAsync);
	...
	it("should throw error when SecureStore throws on removeItem", async () => {
		const mockDeleteItem = jest.mocked(SecureStore.deleteItemAsync);
	...
	it("should log warning for unsupported clear operation", async () => {
		...
		expect(warnSpy).toHaveBeenCalledWith(
			"Clear operation not supported in SecureStore",
		);
